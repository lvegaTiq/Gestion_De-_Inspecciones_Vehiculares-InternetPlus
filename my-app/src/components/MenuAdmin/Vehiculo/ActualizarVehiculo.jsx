...
import vehiculoImg from "../../../assets/img/Admin/VehiculoA.png";
...
function ActualizarVehiculo() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (!respVeh.ok || !respProp.ok || !respCond.ok) {
          throw new Error("Error al consultar información de vehículo");
        }

        const resultVeh = await respVeh.json();
        const resultProp = await respProp.json();
        const resultCond = await respCond.json();

        const vehiculos = resultVeh.data?.docs || [];
        const v = vehiculos.find((item) => item._id === id);
        ...
        setFormData({
          ...
          placa: v.placa || "",
          modelo: v.modelo || "",
          fechaSoat: toInputDate(v.fechaSoat),
          fechaTecno: toInputDate(v.fechaTecno),
          propietarioId: v.propietario?._id || "",
          conductorId: v.conductor?._id || ""
        });
      } catch (error) {
        console.error(error);
        setErrorMsg(error.message || "Error al cargar el vehículo.");
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await resp.json();
      setSuccessMsg("Vehículo actualizado correctamente.");
      ...
      console.error(error);
      setErrorMsg(error.message || "Error al actualizar el vehículo.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="contentAdmin">
      ...
      <div className="contentAdmin2">
        <div className="registerUser">
          <img src={vehiculoImg} className="img1" alt="Vehículo" />
